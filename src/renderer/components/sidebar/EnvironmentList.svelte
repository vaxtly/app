<script lang="ts">
  ...
  async function toggleActive(envId: string): Promise<void> {
    if (environmentsStore.activeEnvironmentId === envId) {
  ...
  function getContextMenuItems(envId: string) {
    const active = environmentsStore.activeEnvironmentId === envId
    return [
      ...
      { label: active ? 'Deactivate' : 'Activate', action: () => toggleActive(envId), icon: 'M5.636 5.636a9 9 0 1 0 12.728 0M12 3v9' },
      { label: '', action: () => {}, separator: true },
      { label: 'Delete', action: () => handleDelete(envId), danger: true, icon: 'm14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0' },
    ]
  }
</script>
    ...
    {#each filteredEnvironments as env (env.id)}
      ...
      >
        ...
        <button
          ...
          aria-label={active ? 'Deactivate environment' : 'Activate environment'}
          class="flex h-4 w-4 shrink-0 items-center justify-center rounded-full border transition-all duration-150 {active ? '' : 'border-[var(--tint-strong)] hover:border-[var(--tint-bold)]'}"
          style={active ? 'border-color: var(--color-success); background: color-mix(in srgb, var(--color-success) 20%, transparent)' : ''}
        >
          {#if active}
        ...
        <!-- Name (clickable) — activates AND opens tab -->
        <button
          onclick={() => { toggleActive(env.id); onenvironmentclick(env.id) }}
          class="min-w-0 flex-1 truncate text-left text-[13px] {active ? 'font-medium' : 'text-surface-300'}"
          style:color={active ? 'var(--color-success)' : undefined}
        >
          {env.name}
        </button>
